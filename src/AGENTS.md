...
| `claude-models.ts` | Claude model profile：内置 Fable/Opus 登录档与 `glm` API 档，可由 `config.toml` 的 `[claude.models.*]` 覆盖/新增（含 Grok）；`resolveClaudeSdkModel` 返回当前档位的真实 SDK model id。spawn 时第一方 alias 按飞书主力（Fable 或 Opus）+ light Sonnet 5 注入（选 Opus 不注入 Fable），第三方 API 档的四个 alias 全部锁回当前 model。 |

<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-05-31 | Updated: 2026-06-02 -->

# src

## Purpose
`src/` 是 Lodestar daemon 的核心实现层，封装飞书 API、Codex app-server 子进程、每个群的 session 状态机、Card Kit 流式更新、`wt` worktree 群编排、安装/停止/升级 CLI，以及 runtime state 路径和配置读取。

## Key Files
| File | Description |
|------|-------------|
| `session.ts` | 一个飞书群对应一个 `Session`；管理 Codex 生命周期、每轮卡片、消息排队、裸词控制、`wt` worktree 群命令和统计状态。 |
...
| `feishu.ts` | Lark client、tenant token 缓存、群名/会话映射、群创建/解散/成员拉取、消息发送、reaction、附件下载、文件上传和项目目录初始化。 |
| `worktree.ts` | `wt` 的 Git worktree 逻辑：按 `work/*` 分支扫描、创建同级 `<project>[name]` worktree、干净检查和删除目录。 |
...
| `*.test.ts` | Bun 单元测试，覆盖 Card Kit、turn card 渲染、context window 展示和 worktree Git 行为。 |
...
- 修改 `wt` 创建/加入/解散流程后，用 debug 注入在真实群里验证 `wt`、`wt <name>` 和解散按钮；测试结束后清理临时 worktree、群和本地分支。
...
### Internal
- `session.ts` 依赖 `codex-process.ts`、`cardkit.ts`、`cards.ts`、`feishu.ts`、`worktree.ts`、`session-tools.ts`、`session-ask.ts` 和 `session-permission.ts`。

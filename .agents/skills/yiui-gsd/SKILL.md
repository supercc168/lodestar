---
name: yiui-gsd
description: 用白话驱动通用 GSD 多任务工作流；负责 .gsd 任务仓库、TRACKER 跟踪、.planning junction 切换，以及白话到 gsd-* 命令的路由。用户说启动、切换、进度、继续任意 GSD 任务时必须触发，AutoUI 只是其上的一个专用分支。
---
...
- 任何会创建、修改 `.gsd/`、`TRACKER.md`、项目根 `.planning/` junction 的操作
...
- 活跃任务运行时：项目根 `.planning/` 必须是 **Junction**，指向 `.gsd/{active-task-slug}/.planning/`
...
2. **仅**执行 `scripts/bootstrap-autoui-task.ps1 -TaskSlug … -TaskName … -UserBrief …`（脚本原子完成：暂停旧运行中任务、TASK、evidence/milestones/notes、PROJECT 种子、TRACKER、junction、`.gsd` commit）。
...
4. 用户要求继续时，先确认 TRACKER 与 Junction，再使用 `$gsd-progress --next` 推进；用户只要求查看可选动作时使用 `$gsd-next`。
...
- Junction 切换步骤（Windows）：读取本 skill 同目录下的 `extra-junction-bridge.md`

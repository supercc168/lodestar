<!-- Generated: 2026-05-15 | Updated: 2026-07-23 -->
...
- Claude/Codex 共享连续失败工具熔断：同一工具+规范化 input+错误指纹连续命中时，Claude 第 2 次经 `PostToolUseFailure` 注入纠错上下文，第 3 次由 Session 软中断并终结卡片；不自动重放原任务，真人排队消息保留。该机制独立于 Codex-only 的 turn watchdog，禁止用 `maxTurns` 或修改 Grok effort 代替。
...
- `model` 是固定选项：Codex 内建（`gpt-5.6-sol`/`max`）、Claude 第一方（Fable 5 / Opus 5，均 `max`）、GLM 第三方路由（effort 随 config，当前 `xhigh`）、Claude Grok（无痕固定官方 `high`；CatCodex 固定工具兼容 `xhigh`），外加非 Grok 的 `[codex.models.<slug>]` API 档位；effort 锁死、选了即生效，不再动态拉取模型列表；选择按 session+provider 持久化到 XDG data，跨 provider 切换只在空闲或下次启动边界生效，turn 进行中或排队时直接拒绝。
- Grok coding 档位必须走 `[claude.models.grok]` / `[claude.models.grokcc]` 的 Claude Agent SDK 后端，禁止从 `[codex.models.*]` 暴露或启动；旧 `codex:grok*` 持久选择恢复时迁到 Claude，任何真实 model id 为 `grok-*` 的 Codex profile 都会被过滤并在 spawn 边界拒绝。Grok 4.5 官方 effort 只有 `low`/`medium`/`high`，最高且默认是 `high`；无痕路由按官方 `high` 启动，CatCodex 因其 Anthropic 网关只在 `xhigh` 下稳定兑现工具调用而使用兼容参数 `xhigh`。两路都固定 `thinking: disabled`，只关闭 Claude 专属 adaptive 控制，不关闭 Grok 自身 reasoning。API 失败仍需记录并暴露，不能静默换模型或 provider。
...
- `rs` 空闲：Claude 列本机 transcript 会话；Codex 空闲直接恢复/重启当前 thread，不展示 Claude-only 列表。
...
- 操作前先读 `.gsd/TRACKER.md`；任务 STATE 的 canonical 路径是 `.gsd/<slug>/.planning/STATE.md`，底层 GSD 命令必须显式带 `--ws <slug>`。
- 根 `.planning/` 是稳定路由目录：`.planning/workstreams/<slug>` 指向对应 canonical planning；不要把根 `.planning` 当作某个任务的 STATE 目录。
- 允许多个任务同时为运行中；飞书 **gsd 卡**的单字 `选` 只绑定当前 session，暂停/完成/继续只作用于所选 slug，不得隐式暂停其它任务。继续时以磁盘为准，勿重复创建冲突任务。

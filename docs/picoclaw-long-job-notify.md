...
## 1.1 变体：等确认也不回群（2026-08-20）

同一缺口的另一面：**不是长任务跑完不吭声，是「要你拍板」的确认卡根本没进群。**

本机实证：钉钉「重启进程」→ agent 查 skill、写临时脚本、被 safety guard 拦、烧 ~30 次 tool → 群里只有占位「处理中…」。确认卡要人追问「如何了？」才作为最终回复出现。

根因仍是「最终回复依赖模型收尾」。收尾失败（烧轮次 / 被拦 / 半死）= 群里静默。修法相同：**确认卡也由脚本直推**，不经过模型最终消息。本机对应脚本是 `restart_ask.sh`（只读 ASK）+ `restart_do.sh`（给了编号才 stop/start）。

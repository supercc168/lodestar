...
- 任务从规划、执行、审核到人工确认合并都会把结果写回任务评论区；失败原因会直接暴露，不再只停留在本地日志里。
- 自动执行和审核流程更稳：审核完成后需要人工勾选任务才会触发本地合并，避免任务清单自己更新时误触发下一步。

**Full Changelog**: https://github.com/leviyuan/lodestar/compare/v0.8.3...v0.9.0

## v0.8.3 · 2026-06-10

- Codex 新对话启动和轮次编号更稳，首次回复不再容易显示错轮或继承旧计数。
- 工作群加载本地 `AGENTS` 指令的匹配更准确，非 Git 场景启动 Codex 也更可靠。
- 卡片刷新更稳：`id_convert` 短暂空结果会重试，正文和 footer 改为整段更新，减少闪烁和打字流卡顿。

**Full Changelog**: https://github.com/leviyuan/lodestar/compare/v0.8.2...v0.8.3

 ...
 *   5. mid-flight interrupt                  — two messages back-to-back
...
await waitIdle(session, 90_000, 'phase3')
await sleep(2000)
...
await waitIdle(session, 90_000, 'phase4')
await sleep(3000)

// 5) mid-flight interrupt
await announce('[5/8] 中途打断：发一条慢任务，1.5s 后再发一条新任务')
session.onUserMessage('请用中文逐字数 1 到 50（每个数字独立一行），慢慢说。')
await sleep(1500)
await session.onUserMessage('好了别数了，换个话题：用一句话告诉我今天日期。')
await waitIdle(session, 60_000, 'phase5')
await sleep(2000)
...
await waitIdle(session, 90_000, 'phase6')
await sleep(2000)

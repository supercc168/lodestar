...
- `model` 是固定选项：Codex 内建（`gpt-5.6-sol`/`max`）、Claude 第一方（Fable 5 / Opus 5，均 `max`）、GLM 第三方路由（effort 随 config，当前 `xhigh`）、Claude Grok（无痕固定官方最高 `xhigh`；CatCodex 固定网关兼容 `xhigh`），外加非 Grok 的 `[codex.models.<slug>]` API 档位；effort 锁死、选了即生效，不再动态拉取模型列表；选择按 session+provider 持久化到 XDG data，跨 provider 切换只在空闲或下次启动边界生效，turn 进行中或排队时直接拒绝。

<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-05-31 | Updated: 2026-06-04 -->

# src

## Purpose
`src/` 是 Lodestar daemon 的核心实现层，封装飞书 API、Codex app-server 子进程、每个群的 session 状态机、Card Kit 流式更新、模型/effort 持久选择、`wt` worktree 群编排、安装/停止/升级 CLI，以及 runtime state 路径和配置读取。
...
| `*.test.ts` | Bun 单元测试，覆盖 Card Kit、turn card 渲染、card action 回调返回、context window 展示、outbound marker、Codex 事件解析和 worktree Git 行为。 |

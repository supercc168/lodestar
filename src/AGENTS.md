...
| `claude-models.ts` | Claude model profile：内置 Fable/Opus 登录档与 `glm` / `grok` / `grokcc` API 档，可由 `config.toml` 的 `[claude.models.*]` 覆盖/新增；`resolveClaudeSdkModel` 返回当前档位的真实 SDK model id。spawn 时第一方 alias 按飞书主力（Fable 或 Opus）+ light Sonnet 5 注入（选 Opus 不注入 Fable），第三方 API 档的四个 alias 全部锁回当前 model；真实 `grok-*` 还启用省略 effort、禁用 thinking 的 SDK 兼容参数。 |
...
- `model` 命令为固定选项(codex 内建=gpt-5.6-sol/max、claude 第一方=Fable 5/Opus 5 均 max、glm=effort 随 config、Grok 无痕/CatCodex=Claude SDK 兼容档 max),effort 锁死一键生效,不动态拉取 `model/list`；Grok 不得走 Codex。
